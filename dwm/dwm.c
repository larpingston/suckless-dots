...
sethfact(const Arg *arg)
{
	...
	if (!arg || !selmon->lt[selmon->sellt]->arrange || !selmon->sel)
		return;
...
tile(Monitor *m)
{
	...
	for (i = 0, my = ty = m->gappoh*oe, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			r = MIN(n, m->nmaster) - i;
			rem = m->wh - my - m->gappoh*oe - m->gappih*ie * (r - 1);
			if (r > 1 && i == 0 && m->hfact >= 0)
				h = rem * m->hfact;
			else
				h = rem / r;
			...
			r = n - i;
			rem = m->wh - ty - m->gappoh*oe - m->gappih*ie * (r - 1);
			if (r > 1 && i == m->nmaster && m->sfact >= 0)
				h = rem * m->sfact;
			else
				h = rem / r;
